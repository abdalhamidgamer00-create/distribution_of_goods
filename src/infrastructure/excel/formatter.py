...
def _load_openpyxl_dependencies():
    """Import openpyxl lazily so the GUI can start without it installed."""
...
def save_formatted_excel(dataframe: pd.DataFrame, file_path: str) -> None:
    ...
    try:
        ...
        with pd.ExcelWriter(file_path, engine='openpyxl') as excel_writer:
            ...
            _apply_header_styles(worksheet, deps)
            _apply_column_widths(worksheet, dataframe.columns, deps)
            _apply_borders(worksheet, len(dataframe), len(dataframe.columns), deps)
            _apply_conditional_formatting(
                worksheet, len(dataframe), dataframe.columns, deps
            )
            
    except ModuleNotFoundError as error:
        raise ModuleNotFoundError(
            "openpyxl is required to create Excel outputs. "
...
def _apply_header_styles(worksheet, deps) -> None:
    ...
    header_fill = deps["PatternFill"](
        start_color="4472C4", 
        end_color="4472C4", 
        fill_type="solid"
    )
    header_font = deps["Font"](bold=True, color="FFFFFF")
    
    for cell in worksheet[1]:
        cell.fill = header_fill
        cell.font = header_font
        cell.alignment = deps["Alignment"](horizontal='center')


def _apply_column_widths(worksheet, columns, deps) -> None:
    ...
    for index, column_name in enumerate(columns, 1):
        column_letter = deps["get_column_letter"](index)
        width = width_mapping.get(column_name, 12)
        worksheet.column_dimensions[column_letter].width = width


def _apply_borders(worksheet, row_count: int, col_count: int, deps) -> None:
    """Applies thin borders to data cells efficiently."""
    if row_count <= 0 or col_count <= 0:
        return
        
    thin = deps["Side"](style='thin')
    std_border = deps["Border"](left=thin, right=thin, top=thin, bottom=thin)
...
def _apply_conditional_formatting(worksheet, row_count: int, columns, deps) -> None:
    """Applies color scale to balance columns."""
    color_scale = deps["ColorScaleRule"](
