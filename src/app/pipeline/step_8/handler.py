...
def step_8_split_by_product_type(use_latest_file: bool = None) -> bool:
    ...
    transfer_files = _validate_transfers_directory(transfers_base_dir)
...
def _validate_transfers_directory(transfers_base_dir: str) -> list:
...
def _is_split_file(file: str, categories: list) -> bool:
    ...
    return any(file.endswith(f'_{category}.csv') for category in categories) or any(file == f'{category}.csv' for category in categories)
...
def _convert_to_excel(transfers_base_dir: str) -> bool:
    ...
    if not split_files:
        ...
        return False
    return _try_convert_to_excel(split_files, transfers_base_dir, excel_output_dir)
...
def _find_split_csv_files(transfers_base_dir: str) -> list:
    ...
    for root, dirs, files in os.walk(transfers_base_dir):
        for file in files:
            if file.endswith('.csv') and any(file.endswith(f'_{category}.csv') for category in categories):
                split_files.append(os.path.join(root, file))
    return split_files


def _try_convert_to_excel(split_files: list, transfers_base_dir: str, excel_output_dir: str) -> bool:
...
def _count_excel_by_category(excel_output_dir: str) -> dict:
    ...
    for root, dirs, files in os.walk(excel_output_dir):
        for file in files:
            _count_single_excel_file(file, categories, category_counts)
    return category_counts


def _count_single_excel_file(file: str, categories: list, category_counts: dict) -> None:
