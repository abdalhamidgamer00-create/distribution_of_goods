...
def _validate_columns(dataframe, analytics_path: str):
    """Validate analytics columns and return success status."""
    missing_columns = validate_analytics_columns(dataframe)
    if missing_columns:
        logger.error("Missing required columns in %s: %s", analytics_path, missing_columns)
        return False
    return True
...
def _load_and_validate_analytics(branch: str):
    ...
    if dataframe is None or not _validate_columns(dataframe, analytics_path):
        return None, None, None
    return dataframe, has_date_header, first_line


# =============================================================================
# SURPLUS CALCULATION HELPERS
# =============================================================================

def _calculate_branch_surplus(dataframe, branch: str):
...
def _create_csv_and_log(result_dataframe, branch: str, output_dir: str, base_name: str, 
                         timestamp: str, has_date_header: bool, first_line: str) -> dict:
    """Create CSV files and log results."""
    csv_files = generate_csv_files(
        dataframe=result_dataframe, branch=branch, output_dir=output_dir, base_name=base_name,
...
def _generate_branch_files(result_dataframe, branch: str, has_date_header: bool, first_line: str) -> dict:
...
def _process_branch(branch: str) -> dict:
    ...
    dataframe, has_date_header, first_line = _load_and_validate_analytics(branch)
    if dataframe is None:
        return None
    
    result_dataframe = _calculate_branch_surplus(dataframe, branch)
