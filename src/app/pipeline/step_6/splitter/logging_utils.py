...
def log_timing_info(
    timing_stats: dict, total_duration: float, branch_count: int
) -> None:
    """Log timing statistics."""
    if timing_stats:
        msg = (
            "Step 6 timing (seconds): prep=%.2f allocation=%.2f "
            "surplus=%.2f writes=%.2f total=%.2f"
        )
        logger.info(
            msg,
...
def log_split_results(
    ...
    logger.info(
        "File split successfully into %s branch files:", 
        len(output_files)
    )
