...
from src.app.pipeline.step_6.splitter.setup import ensure_branch_directories, setup_and_validate
...
def run_split_and_log(csv_path: str, branches_dir: str, base_filename: str, analytics_dir: str) -> None:
    ...
    log_split_results(output_files, timing_stats, total_duration, branches_dir, analytics_dir)


def execute_split(csv_path: str, csv_file: str, branches_dir: str, analytics_dir: str) -> bool:
...
def try_execute_split(renamed_dir: str, csv_files: list, branches_dir: str, analytics_dir: str, use_latest_file: bool) -> bool:
    """Try to execute split with error handling."""
    try:
        csv_file = select_csv_file(renamed_dir, csv_files, use_latest_file)
        return execute_split(get_file_path(csv_file, renamed_dir), csv_file, branches_dir, analytics_dir)
