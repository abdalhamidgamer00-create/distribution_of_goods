...
from src.app.gui.components import render_file_expander, render_download_all_button
...
def list_files_in_folder(folder_path: str, extensions: List[str]) -> List[dict]:
...
def parse_folder_info(folder_name: str, folder_path: str, prefix: str) -> dict:
...
def get_matching_folders(
    ...
    for folder_name in os.listdir(base_dir):
        ...
        if os.path.isdir(folder_path) and folder_name.startswith(prefix):
            ...
            if not branch_filter or branch_filter == 'all' or info['branch'] == branch_filter:
                folders.append(info)
    return folders
...
def render_file_tabs(
    ...
    for tab, directory, file_ext in [(excel_tab, excel_dir, ".xlsx"), (csv_tab, csv_dir, ".csv")]:
        with tab:
