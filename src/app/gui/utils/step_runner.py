...
def run_step_with_dependencies(step_id: str, use_streamlit: bool = True) -> tuple:
    ...
    if not valid:
        return False, result
    target_step_number = result
    all_steps = [step for step in AVAILABLE_STEPS if int(step['id']) <= target_step_number]
    if not all_steps:
        return False, f"لم يتم العثور على خطوات حتى الخطوة {step_id}"
    return _run_with_streamlit_ui(all_steps, step_id, target_step_number) if use_streamlit else _run_without_ui(all_steps, step_id)
...
# =============================================================================
# UI EXECUTION HELPERS
# =============================================================================

def _run_with_streamlit_ui(all_steps: list, step_id: str, target_step_number: int) -> tuple:
    """Run steps with Streamlit progress bar."""
    import streamlit as st
    progress_bar = st.progress(0)
    status_text = st.empty()
    for step_index, step in enumerate(all_steps):
        success, message, step_name = _process_step_with_ui(step, progress_bar, status_text, step_index, len(all_steps))
        if not success:
            return _cleanup_and_return_failure(progress_bar, status_text, step, step_name, message)
    return _cleanup_and_return_success(progress_bar, status_text, step_id, target_step_number)
...
def _process_step_with_ui(step: dict, progress_bar, status_text, step_index: int, total: int):
...
def _cleanup_and_return_success(progress_bar, status_text, step_id: str, target_step_number: int) -> tuple:
    """Cleanup UI elements and return success tuple."""
    progress_bar.empty()
    status_text.empty()
    target_step_name = STEP_NAMES.get(step_id, AVAILABLE_STEPS[target_step_number - 1]['name'])
