...
def show_metrics():
    ...
    files_column, branches_column = st.columns(2)
    
    output_dir = os.path.join("data", "output")
    if os.path.exists(output_dir):
        file_count = sum([len(files) for root, directories, files in os.walk(output_dir)])
        files_column.metric("عدد الملفات", file_count)
    else:
        files_column.metric("عدد الملفات", 0)
    
    branches_column.metric("عدد الفروع", 6)
...
def _get_sorted_excel_files(input_dir: str) -> list:
    ...
    if not os.path.exists(input_dir):
        return None
    excel_files = [filename for filename in os.listdir(input_dir) if filename.endswith(('.xlsx', '.xls'))]
    excel_files.sort(key=lambda filename: os.path.getmtime(os.path.join(input_dir, filename)), reverse=True)
    return excel_files
...
def _display_latest_file_info(input_dir: str, latest_file: str) -> None:
    ...
    st.caption(f"آخر تعديل: {file_modification_time.strftime('%Y-%m-%d %H:%M')}")
...
def show_file_management():
    ...
    upload_column, latest_column = st.columns(2)
    with upload_column:
...
def _render_nav_buttons(step_id: str, buttons: list) -> None:
    ...
    for button_index, (label, page) in enumerate(buttons):
        with columns[button_index]:
            if st.button(label, key=f"nav_{step_id}_{button_index}", type="primary", use_container_width=True):
                st.switch_page(page)
...
def show_results_navigation():
    ...
    columns = st.columns(len(NAV_BUTTONS))
    for column, (label, key, page) in zip(columns, NAV_BUTTONS):
        with column:
...
def show_steps():
    ...
    columns = st.columns(len(visible_steps))
    for step_index, step in enumerate(visible_steps):
        with columns[step_index]:
