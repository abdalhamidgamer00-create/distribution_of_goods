"""Header validation constants."""

def get_required_headers() -> list:
    """Get list of required column headers."""
    return [
        "كود", "إسم الصنف", "سعر البيع", "الشركة",
        "الوحدة", "مبيعات الادارة", "رصيد الادارة",
        "مبيعات الشهيد", "رصيد الشهيد",
        "مبيعات العشرين", "رصيد العشرين",
        "مبيعات العقبى", "رصيد العقبى",
        "مبيعات النجوم", "رصيد النجوم",
        "مبيعات الوردانى", "رصيد الوردانى"
    ]


def get_optional_headers() -> list:
    """Get list of optional column headers."""
    return [
        "متوسط مبيعات الادارة", "متوسط مبيعات الشهيد",
        "متوسط مبيعات العشرين", "متوسط مبيعات العقبى",
        "متوسط مبيعات النجوم", "متوسط مبيعات الوردانى",
        "إجمالى المبيعات", "إجمالى رصيد الصنف"
    ]
