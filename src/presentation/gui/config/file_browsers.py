...
FILE_BROWSERS = {
    'transfers': {
        ...
        'icon': '📤',
        'help_text': (
            'تعرض ملفات التحويلات الفردية المباشرة التي تتم بين فرع وآخر '
            'بناءً على الاحتياج.'
        ),
    ...
    'surplus': {
        ...
        'icon': '📦',
        'help_text': (
            'تقارير توضح أصناف الفائض التي لا تزال موجودة في الفروع '
            'بعد تلبية احتياجات الفروع الأخرى.'
        ),
    ...
    'shortage': {
        ...
        'icon': '⚠️',
        'help_text': (
            'تقارير توضح احتياجات الفروع التي لم يتم تغطيتها بالكامل '
            'من خلال التحويلات المتاحة.'
        ),
    ...
    'merged': {
        ...
        'icon': '📋',
        'help_text': (
            'ملفات التحويل النهائية التي تدمج التحويلات المباشرة مع '
            'عمليات إعادة توزيع الفائض لتعظيم الفائدة.'
        ),
        'csv': os.path.join(
    ...
    'separate': {
        ...
        'icon': '📂',
        'help_text': (
            'تفاصيل التحويلات لكل صنف على حدة ناتجة عن عملية '
            'الدمج وإعادة التوزيع.'
        ),
        'csv': os.path.join(
        ...
        'step': 11, 
        'session_key': 'separate_selected_source', 
        'key_prefix': 'separate',
        'category': 'separate'
    },
    'sales_analysis': {
        ...
        'icon': '📈',
        'help_text': (
            'نتائج فحص وتدقيق بيانات المبيعات الخام قبل البدء '
            'في عملية التوزيع.'
        ),
    ...
    'collections': {
        ...
        'icon': '📦',
        'help_text': (
            'تحتوي هذه الملفات على تجميعة لكافة التحويلات الخارجة من '
            'الفرع إلى جميع الفروع الأخرى، مصنفة حسب نوع المنتج، '
            'وذلك دون إجراء أي عمليات نقل للفائض المتبقي.'
        ),
        'csv': os.path.join("data", "output", "transfers", "csv"),
        'excel': os.path.join("data", "output", "transfers", "excel"),
        'step': 7, 
