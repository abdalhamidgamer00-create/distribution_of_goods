...
def update_analytics_data(
    branch: str, product_index: int, withdrawals_list: list, 
    analytics_data: dict, max_withdrawals: int
) -> int:
    """Update analytics data with new withdrawals."""
    existing_list = analytics_data[branch][1]
    while len(existing_list) < product_index:
        existing_list.append([create_empty_withdrawal_record()])
    existing_list.append(withdrawals_list)
...
def fill_empty_product_records(
    branches: list, analytics_data: dict, num_products: int
) -> None:
    ...
    for branch in branches:
        ...
        for idx in range(len(existing_list), num_products):
            if branch_df.iloc[idx]['needed_quantity'] <= 0:
                existing_list.append([create_empty_withdrawal_record()])
