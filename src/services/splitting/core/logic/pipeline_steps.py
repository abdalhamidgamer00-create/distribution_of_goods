...
from src.services.splitting.processors.withdrawal_processor import (
    process_withdrawals
)
from src.services.splitting.surplus import redistribute_wasted_surplus
from src.services.splitting.core.analytics import (
    initialize_analytics_data, fill_empty_product_records
)
...
def run_processing_pipeline(
    ...
    res = _process_surplus_distribution(
        branches, 
        branch_data, 
    ...
    analytics_data, all_withdrawals, max_withdrawals = res
    
    final_surplus_rem_dict = calculate_surplus_remaining(
        branches, branch_data, all_withdrawals
    )
    processed_data = process_withdrawals(
        branches, analytics_data, max_withdrawals, final_surplus_rem_dict
    )
    return processed_data, max_withdrawals


def _process_surplus_distribution(
    branches: list, 
    branch_data: dict, 
    proportional_allocation: dict, 
    num_products: int, 
    ...
    res = redistribute_wasted_surplus(
        branches, 
        branch_data, 
        analytics_data, 
        all_withdrawals, 
    ...
    max_withdrawals, timing_stats["second_round_time"] = res
