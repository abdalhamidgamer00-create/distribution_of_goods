...
def _get_branch_product_data(analytics_data: dict, branch: str, product_index: int) -> tuple:
...
def _calculate_transferred_amount(analytics_data: dict, branch: str, product_index: int) -> float:
...
def _calculate_branch_eligibility(branch: str, analytics_data: dict, product_index: int, 
                                   balance_limit: float) -> tuple:
    """Calculate if a branch is eligible and its details."""
    balance, needed, avg_sales = _get_branch_product_data(analytics_data, branch, product_index)
    transferred_so_far = _calculate_transferred_amount(analytics_data, branch, product_index)
...
def _find_eligible_branches(branches: list, analytics_data: dict, product_index: int, balance_limit: float) -> list:
    """Find branches eligible for second-round redistribution."""
    eligible = []
    for branch in branches:
        result = _calculate_branch_eligibility(branch, analytics_data, product_index, balance_limit)
        if result:
...
# =============================================================================
# SURPLUS AVAILABILITY HELPERS
# =============================================================================

def _check_surplus_availability(branch_data: dict, other_branch: str, product_index: int, 
                                  all_withdrawals: dict, remaining_capacity: float) -> float:
    """Check availability and return transfer amount or 0."""
    available_surplus = calculate_available_surplus(branch_data, other_branch, product_index, all_withdrawals)
...
def _record_redistribution(branch: str, other_branch: str, product_index: int, transfer_amount: float,
...
def _execute_transfer(branch: str, other_branch: str, product_index: int, transfer_amount: float,
    ...
    max_withdrawals = _record_redistribution(
        branch, other_branch, product_index, transfer_amount,
        available_surplus, analytics_data, all_withdrawals, max_withdrawals
    )
    
    logger.debug(f"Second round: Transferred {transfer_amount:.2f} of product {product_index} from {other_branch} to {branch}")
    return max_withdrawals, 1, remaining_capacity - transfer_amount


def _try_redistribute_from_branch(other_branch: str, branch: str, product_index: int, remaining_capacity: float,
    ...
    available_surplus = calculate_available_surplus(branch_data, other_branch, product_index, all_withdrawals)
    transfer_amount = _check_surplus_availability(branch_data, other_branch, product_index, all_withdrawals, remaining_capacity)
    
    if transfer_amount <= 0:
        return max_withdrawals, 0, remaining_capacity
    
    return _execute_transfer(branch, other_branch, product_index, transfer_amount, available_surplus, remaining_capacity, analytics_data, all_withdrawals, max_withdrawals)
...
def _process_all_other_branches(branch: str, product_index: int, remaining_capacity: float, branches: list,
                                 branch_data: dict, analytics_data: dict, all_withdrawals: dict,
                                 max_withdrawals: int) -> tuple:
    ...
    for other_branch in branches:
        if not _should_skip_branch(other_branch, branch, remaining_capacity):
            max_withdrawals, count, remaining_capacity = _try_redistribute_from_branch(
                other_branch, branch, product_index, remaining_capacity, 
...
def _redistribute_to_branch(branch: str, product_index: int, remaining_capacity: float, branches: list,
                            branch_data: dict, analytics_data: dict, all_withdrawals: dict,
                            max_withdrawals: int) -> tuple:
    """Attempt to redistribute surplus to a single branch."""
    return _process_all_other_branches(branch, product_index, remaining_capacity, branches,
                                       branch_data, analytics_data, all_withdrawals, max_withdrawals)
...
def _process_single_product(product_index: int, branches: list, branch_data: dict, analytics_data: dict,
                             all_withdrawals: dict, max_withdrawals: int, balance_limit: float) -> tuple:
    """Process redistribution for a single product."""
    redistributed_count = 0
    for branch, _, _, remaining_capacity in _find_eligible_branches(branches, analytics_data, product_index, balance_limit):
        max_withdrawals, count = _redistribute_to_branch(branch, product_index, remaining_capacity, branches, branch_data, analytics_data, all_withdrawals, max_withdrawals)
        redistributed_count += count
    return max_withdrawals, redistributed_count
